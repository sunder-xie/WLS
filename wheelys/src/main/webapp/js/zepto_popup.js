/*基于zepto的基本弹窗(设置弹窗隐藏 宽高即可使用)不兼容火狐IE,手机专用
黄绩恒 2015-12-17修改*/
;(function($, win, undefined){
	...
	$.fn.closePopup = function(options){//$.closePopup关闭
		...
		this.animate(popupInitStyl[options.animateType], options.duration, options.easing, options.closeCallback, options.delay);
		if (hasMask) {
			$cover.off('click');
			$cover.remove();
			/*setTimeout(function(){
	...
	$.fn.popup = function(options){//$.popup弹出
		...
		if(options.closeCallback)defaultOptions.closeCallback = options.closeCallback
